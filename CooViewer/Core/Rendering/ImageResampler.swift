...
/// EN: Pre-resamples pages to exact display pixels (CG high-quality down,
/// EN: optional MetalFX up) so layers can show them 1:1; small LRU cache.
actor ImageResampler {
    ...
    /// EN: Returns the image resampled to pixelSize, cached; falls back to CG
    /// EN: when MetalFX is unavailable or fails.
    ...
    /// EN: CPU resample via CGContext .high interpolation; also used to
    /// EN: normalize images to RGBA before Metal texture upload.
    static func cgResample(_ image: CGImage, width: Int, height: Int) -> CGImage? {
        // グレースケール/CMYK は RGBA コンテキストを作れないため sRGB へ変換する
        // EN: Non-RGB sources (gray/CMYK) fall back to sRGB so context creation
        // EN: succeeds and the HQ pipeline is not silently skipped.
