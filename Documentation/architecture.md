...
| 中・サイドボタンの微動クリック | 30pt 超でドラッグ判定されても、中・サイド等(button >= 2)でドラッグ割当が無ければ、ディスパッチ層で元の修飾キー・左右半面を保ったクリックへ戻す。HUD もクリック割当名を予告する。左右ボタン(0/1)のドラッグジェスチャ/ドラッグスクロール意味論、割当済みドラッグの優先、カール確定後の二重発火防止、1 秒超の長押しキャンセルは維持。状態機械 `MouseGestureRecognizer` は変更しない(仕様書 §5.9、bd cooViewer-dgev) |
