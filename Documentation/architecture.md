...
| ページめくり効果 | 旧実装には無い**新規**(既定オフ)。defaults `PageTurnAnimation`(0=なし/1=フェード/2=スライド/3=ズームフェード/4=フリップ)。フェード/スライドは CATransition(スライドは読み方向連動: 右→左読みで進むと新ページが左から入る。PageTurnAnimation.entersFromLeft)、ズームフェードは container への軽い拡大+フェード、フリップは進入側エッジを軸にした 3D 回転(キーフレームの行列合成。ページ回転表示中は省略)。適用はページ送り(次/前/半ページ・スライドショー)のみで、ジャンプ・設定変更の再表示には付けない(ReaderWindowController.pendingTurnForward の消費方式)。システムの「視差効果を減らす」で自動無効。設定「表示」ペインと表示メニューの両方から切替(§7.5 の不変条件)。検証フラグ `--then-next-page` |
