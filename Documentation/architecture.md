...
| レトロ日本形式 (MAG/MAKI) | 旧実装には無い**新規**。RetroImageDecoding(Core)が MAG(MAKI02。16/256 色、パディング/切出し、機種別パレットビット: X68K は機種名文字列でも 5bit 判定、1:2 アスペクト伸長。MSX YJK は対応外で nil)と MAKI01A/B(640x400 固定、4x4 マスク+XOR フィルタ、パレットは「非 0 上位ニブル \| 0x0F」規則)をデコード。**判定は拡張子でなく先頭マジック**(MAKI02/MAKI01A/MAKI01B)— .max(3ds Max)等の同拡張子別形式は誤描画しない。ImageIO 失敗時のフォールバックとして ImageDecoding から呼ぶ。検証: Maki-chan 仕様文書+公式サンプル 9 点とのピクセル照合(テストは RETRO_SAMPLE_DIR 指定時のみゴールデン比較) |
