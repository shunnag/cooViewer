...
- ~~マウス/ホイール割り当ての編集 UI~~ → **実装済み**(設定「マウスとジェスチャ」ペイン、SettingsSelectedTab 9)。**「できること別」方式**: 機能(マウスアクション 0-64、41 を除く)をカテゴリ(ページをめくる/しおり/本・フォルダ/スクロール/表示/ツール/ファイル・ウインドウ)ごとに列挙し、各行で入力(クリック/ドラッグ/ジェスチャ、修飾キー可)をチップとして追加・削除する — macOS のキーボードショートカット設定と同じ向き。1 機能に複数入力可、使用中の入力を選ぶと付け替え(シートで予告)。数値は機能単位で全割当行へ適用、switchAction(可否は旧設定シートのリスト準拠)はチップのメニュー内。表示モード別の上書きは折りたたみ(トリガ中心+継承行を「基本設定のとおり」で可視化)。UI⇄配列の写像は MouseBindingCatalog(純関数)で、旧互換形式 §5.1 で書き戻す。ホイールは従来どおり「操作」ペインの CanScrollMode/感度で設定。
...
| フルスクリーン | 疑似(hidesOnDeactivate)→ ネイティブ。esc で解除、3 勘所(§13.2)は再現。起動ウインドウ状態は 3 択(常にウインドウ/常に全画面/前回の状態)。新 Int キー `LaunchWindowMode`(0/1/2、既定 0)、前回状態は新 Bool キー `LastWindowWasFullscreen`(既定オフ)に全画面の出入りで記録し、終了時の実ウインドウ状態を最終値とする。旧 `Fullscreen` キーは読まない(1.x が読み書きし既定 YES。仕様書 §6.1。再利用すると 1.x の挙動を変え、旧ユーザーの初回「前回の状態」が全て全画面になるため) |
| マウスクリックのモード解決 | fitScreenMode 3 のとき Mode2 参照(§5.3)→ キーと同じ Mode3 参照に統一 |
| マウス操作の左右半面判定 | 旧: クリック=window contentView frame / ジェスチャ・マルチタッチ=imageView frame の不統一(§5.9。スクロール中にズレる)→ **ReaderView bounds の中央線に統一**。ドラッグジェスチャ・マルチタッチにも位置(leftHalf)を渡し positional 系アクションの割当が旧同様に位置依存で効く |
| 中・サイドボタンの微動クリック | 30pt 超でドラッグ判定されても、中・サイド等(button >= 2)でドラッグ割当が無ければ、ディスパッチ層で元の修飾キー・左右半面を保ったクリックへ戻す。HUD もクリック割当名を予告する。左右ボタン(0/1)のドラッグジェスチャ/ドラッグスクロール意味論、割当済みドラッグの優先、カール確定後の二重発火防止、1 秒超の長押しキャンセルは維持。状態機械 `MouseGestureRecognizer` は変更しない(仕様書 §5.9、bd cooViewer-dgev)。EPUB モードでも otherMouse* をローカルモニタで拾い同一規則で発火(bd cooViewer-pex7) |
| ピンチ/回転の発火 | 旧: 累積 ±0.2 / ±15° に到達した瞬間に 1 ジェスチャ 1 回発火(§5.9)→ **ジェスチャ終了時に累積 ±0.05 / ±5° で判定**(2.0b 系でチューニングした現行値と発火タイミングを維持。逐次発火しない点は同等) |
...
| サイドボタンの既定割当 | 旧既定(§5.7.4)に無い**新規**: button 3=前の本(15)/button 4=次の本(14)。当初 2.0b の既定は前/次ページだったが、2026-09 のユーザー要望(同一フォルダ内の書庫移動。ページ送りはクリック・キー・スワイプで可能)により変更。switchAction なし=ブラウザの戻る/進むと同じ論理ナビゲーションとして綴じ方向非依存。保存済み MouseArray へは起動時のメモリ内注入(該当ボタンが未使用のときだけ)。**設定「マウスとジェスチャ」で MouseArray を編集・保存すると 2.0 専用フラグ `MouseArrayUserEdited`(legacy スキーマ外の新キー)が立ち、以後は注入せず保存内容だけを使う** — UI で削除したサイドボタン行は復活しない。保存時は表示中の配列(注入行を含む)をそのまま旧互換形式で書き戻す(編集後はそれがユーザーの明示した内容になるため) |
...
| 拡大中のページ送り演出 | ピンチ拡大中に次/前ページへ送ると、**まずズームを 1.0 へ滑らかに戻し(既存の 120Hz 慣性機構 animateZoomOut)、フィット表示になってから設定のめくり演出(カール/スライド/フェード)を実行**する「引いてからめくる」2 段階(ReaderWindowController.turningPage が nextPage/previousPage/halfNext/halfPrevious を包む)。ズーム中はカールが抑止されるため、この順序でカール演出も正しく効く。等倍時・視差効果オフ時は即実行(従来と同一)。**めくり効果が「なし」のときはこのズームアウト演出も付けず即座に送る**(「効果なし」を一貫させる。turningPage が pageTurnAnimation==.none を判定)。実装は zoomSettleCompletion で settle 完了後に送り本体へ継続 |
| 水平スワイプのバインディング解決 | 「スワイプでページめくり」トグル(SwipeToTurnPage)と向き反転(FlipSwipeDirection)は、**水平スワイプの割当が次/前ページのときだけ**適用する(handleGesture が resolveMouse で判定)。水平スワイプを別アクション(次の本など)へ割り当てたユーザーの設定はトグル/反転の影響を受けず常に発火 |
| スマートズーム/Force click | 旧実装には無い**新規**(バインディング体系の外の固定動作、それぞれ defaults `SmartZoomEnabled`/`ForceClickLoupe` で無効化可・既定オン)。スマートズーム=2 本指ダブルタップで、全体フィットならタップ位置を中心に幅フィットへ、拡大系表示中なら全体フィットへ戻るトグル(Preview.app と同じ心理モデル。受信時にピンチの累積値をリセットして誤発火を防ぐ)。Force click=**クイックルーペ**: 深押し(stage 2 遷移)している間だけルーペを表示し、離すと消える(触覚フィードバック付き。消す操作を覚える必要がない quasimode)。⌘L / l キー / 中クリックの常時表示トグルは従来どおりで、常時表示中に深押しした場合はそれを消す。発火した押下の解放ではクリック/ジェスチャを起こさない(二重発火防止) |
...
| サムネイル一覧のグリッド | 旧: 設定の行×列固定(§6.1 Thumbnail{row,column}・§4.8)→ **セルサイズ基準の自動グリッド(Photos 風)**: ウインドウサイズとセルサイズ(defaults `ThumbnailCellSize`、80–400pt・既定 160)から行列を自動算出(`ThumbnailGridLayout.dimensions`)し、オーバーレイ上のピンチで連続拡縮(指を離した値を保存し次回以降も維持。ジェスチャ中はセル固定サイズで指に追従、静止時は余白を均等吸収)。画面送り・§4.8 のページ単位閲覧・見開きセル・EPUB の画面単位一覧はそのまま(スクロール化はしない — 「行×列が決まる」ページめくり閲覧が旧仕様の芯のため)。ズーム/リサイズをまたぐ表示位置は先頭セル組アンカーで維持(逐次換算だと床関数の丸めがピンチ中に累積し先頭へドリフトする)。旧キー Thumbnail{row,column} は 1.x 用に凍結(以後読み書きしない。新キーは追加のみで §13.5 の移行不要)。設定「表示 > サムネイル」は行/列ステッパー → サイズスライダー(applySettings 経由で開いている一覧へ即時反映)。検証: `-ThumbnailCellSize n --show-thumbnails` |
...
| ウインドウ位置の復元 | サイズも位置も AppKit の frame autosave("ReaderWindow")で復元する。AppKit は復元時に constrainFrameRect でウインドウを必ず画面内へ収める(解像度変更・ディスプレイ取り外し後も画面外に出ない。隔離クローンで実測)ため、**初回起動(保存フレームがまだ無い)のときだけ中央**へ置き、以降は autosave の復元に委ねる。以前は終了時に記録した画面解像度(defaults `ReaderWindowScreenSize`)が現在の画面と一致するときだけ位置を復元していたが、不一致時に呼ぶ window.center() が autosave の保存位置を上書きしてしまい、クラッシュ/強制終了で解像度記録が残らないと毎回中央へリセットされる不具合があったため撤去した(b19 の「位置が保存されない・リセットされる」報告)。**macOS 26 のタイル状態はフレームだけ復元し、タイルとしては復活させない**: タイル(フィル等)のまま終了すると autosave 文字列に tilingState JSON が付く。そのまま復元すると WindowManager がタイルを再確立して保存し直すため、以後ウインドウを動かしても古いフィルが毎回蘇る(自己永続化)。起動時に文字列を検査し、tilingState があれば JSON だけ除去してから setFrameAutosaveName を呼ぶ(ReaderWindowController.untileSavedWindowFrame)。フレーム欄は保存値のままなので、フィルで終了すれば次回も同じ見た目で起動し、外して終了すればその位置で起動する |
...
│   │   │                              幅 4 並列、登録は候補順に直列)
│   │   ├── EPUBSource.swift        — actor。固定レイアウト EPUB(Washi。単一画像
│   │   │                             ページの直接デコード+複雑ページの WebKit
│   │   │                             ラスタライズ。§2.4 EPUB 対応)
...
├── UI/
│   ├── Reader/
│   │   ├── ReaderWindowController.swift(+Input/+Library/+Thumbnails/+EPUB 拡張。
│   │   │                             +EPUB はリフロー EPUB の同ウインドウ表示モード)
│   │   │                           — 開くフロー・表示更新・入力ディスパッチ・
...
| 先読み | `Book.schedulePrefetch`: 深さは**メモリ条件に自動追従**(PreresamplePolicy.decodeAhead/decodeBehind、refreshDisplay で実ページサイズから更新): 進行方向 = max(リサンプル先読み実効数+4, PageCache 予算の 1/3 ÷ デコード済みページバイト数)を媒体別下限(SSD 12/HDD 16/ネットワーク 20)〜64 に丸め、逆方向 = 進行方向の 1/4(3-16)。「+4」は事前リサンプルが常にデコード済みへ命中し I/O と ML 計算が直列化しないため。高度設定 ON は明示値(2-64/0-16)を尊重。ジャンプでキャンセル。`supportsParallelPageLoads` なソース(フォルダ)は 4 並列デコード | — |
