...
| 圧縮ノイズ低減 | 旧実装には無い**新規**(既定オフ)。古い JPEG のブロックノイズ向け。設定「表示」の 2 軸: 強度(なし/弱/中/強/最高。defaults `NoiseReductionLevel` 0-4)と適用範囲(メイン表示のみ/メイン表示とルーペ/原寸表示も含めて全て。`NoiseReductionScope`)。**JPEG のページのみ対象**(PNG 等の線画を甘くしないため拡張子判定)。弱(0.035/0.5)・中(0.06/0.35)は CINoiseReduction(Metal)。**強は CoreML**: waifu2x のノイズ除去モデル anime_noise2(MIT、約 1.2MB)を **初回選択時の同意後・必要時にのみ**配布元(imxieyi/waifu2x-mac)から DL し、SHA-256 ピン照合→コンパイル→Application Support/Models にキャッシュ(MLNoiseReducer actor。入力 3×142×142/出力 conv7、128px タイル+7px 文脈、1 タイル約 2ms)。**最高は CoreML の ×4 超解像**: Real-ESRGAN x4plus anime 6B(BSD-3-Clause)を自前で CoreML 変換(`Scripts/convert-realesrgan.py`、fp16 で約 9MB、PyTorch とのパリティ最大誤差 0.003 確認済み)し、**本リポジトリのリリース資産(models-1 タグ)**から同意後 DL(MLSuperResolver actor。入力 [1,3,256,256] の 0-1 → 出力 [1,3,1024,1024]、内容 240px タイル+8px 文脈マージン、1 タイル約 46ms)。リサンプル前段で 4 倍化し後段の表示縮小で画質向上を得る設計のため、**等倍系(ルーペ・原寸)では強へ格下げ**(cappedForOriginalSize)し、**長辺 2048px 超の元画像も強へフォールバック**(恩恵が小さく時間だけ伸びるため)。タイル継ぎ目は「マージン捨て」だけでは不十分(GAN は平坦部のトーンがタイル毎に Δ1-2 階調揺れ、帯として見える。実測): 右・下へ margin 分を余計に書き、次のタイルが**線形フェザーで合成**+8bit 量子化の残段差は **Bayer 8×8 の秩序ディザ**で分散(いずれも決定的処理)。処理が重いため結果は HEIC でディスクキャッシュ(Caches/jp.coo.cooViewer/SuperRes/、キー=リサンプルキー+元サイズの SHA-256、サムネイルと同じ保持日数で起動時トリム)。モデルの取得・SHA 照合・コンパイル・ロードは **MLModelInstaller** が強・最高で共通(状態は MLModelInstallStatus.noise / .superResolution)。未導入・失敗・XCTest 時は 1 段ずつフォールバック(最高→強→中相当の CI)。設定の「強(ML モデル)」「最高(×4 ML 超解像)」はそれぞれ初回に DL と処理負荷の確認ダイアログを出し、OK の場合のみ設定される(`NoiseReductionMLAccepted` / `NoiseReductionSRAccepted`)。導入状態は設定にキャプション表示。メイン表示は ImageResampler のリサンプル前段で適用しレベル込みのキーでキャッシュ(事前リサンプルも同一条件)、ルーペは超解像前、原寸はフル解像度デコード後に適用 |
