...
| 圧縮ノイズ低減 | 旧実装には無い**新規**(既定オフ)。古い JPEG のブロックノイズ向けに CINoiseReduction(Metal)を表示パイプラインへ挿入する。設定「表示」の 2 軸: 強度(なし/弱/強。defaults `NoiseReductionLevel`)と適用範囲(メイン表示のみ/メイン表示とルーペ/原寸表示も含めて全て。`NoiseReductionScope`)。**JPEG のページのみ対象**(PNG 等の線画を甘くしないため拡張子判定)。メイン表示は ImageResampler のリサンプル前段で適用しレベル込みのキーでキャッシュ(事前リサンプルも同一条件で先行計算)、ルーペは超解像前、原寸はフル解像度デコード後に適用。パラメータ: 弱=noiseLevel 0.02/sharpness 0.6、強=0.05/0.4 — 小振幅の圧縮段差は均し、本物のエッジは保持・鮮鋭化される(合成ブロックノイズでの実効性テストあり) |
