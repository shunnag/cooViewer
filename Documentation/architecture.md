...
| ウインドウ位置の復元 | サイズは AppKit の frame autosave("ReaderWindow")で常に復元。位置は**終了時(⌘Q/ウインドウクローズ)に記録した画面解像度(screen.frame サイズ、defaults `ReaderWindowScreenSize`)が現在接続中のいずれかの画面と一致するときだけ**復元し、不一致・初回起動時は従来どおり中央に置く(旧実装は "NormalWindow" autosave のみで解像度照合なし。autosave 文字列内の画面欄はメニューバー/Dock 状態に依存するため照合に使わない)。**macOS 26 のタイル状態は復元しない**: タイル(フィル等)のまま終了すると autosave 文字列に tilingState JSON が付き、そのまま復元すると WindowManager がタイルを再確立して全画面フィルで起動し、終了時に再びタイル込みで保存される(自己永続化)。起動時に文字列を検査し、tilingState があればフレーム欄をタイル前の untiledFrame に差し替えて JSON を除去してから setFrameAutosaveName を呼ぶ(ReaderWindowController.untileSavedWindowFrame) |
