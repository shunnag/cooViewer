...
| コレクションフォルダの自動オープン | 旧: 画像ゼロのフォルダは開くのを拒否(§4.1.2 手順 3)→ 中の最初の本を自動で開く。**前の本への移動で入った場合は最後の本**(読書順の平坦化)。統合ソース(書庫/PDF 入りフォルダ)が 0 ページの場合は組み立て失敗なのでドリルしない(中の書庫を単体で開くと階層がずれるため) |
