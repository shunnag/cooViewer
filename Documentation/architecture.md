...
| ウインドウ位置の復元 | サイズは AppKit の frame autosave("ReaderWindow")で常に復元。位置は**終了時(⌘Q/ウインドウクローズ)に記録した画面解像度(screen.frame サイズ、defaults `ReaderWindowScreenSize`)が現在接続中のいずれかの画面と一致するときだけ**復元し、不一致・初回起動時は従来どおり中央に置く(旧実装は "NormalWindow" autosave のみで解像度照合なし。autosave 文字列内の画面欄はメニューバー/Dock 状態に依存するため照合に使わない)。**macOS 26 のタイル状態はフレームだけ復元し、タイルとしては復活させない**: タイル(フィル等)のまま終了すると autosave 文字列に tilingState JSON が付く。そのまま復元すると WindowManager がタイルを再確立して保存し直すため、以後ウインドウを動かしても古いフィルが毎回蘇る(自己永続化)。起動時に文字列を検査し、tilingState があれば JSON だけ除去してから setFrameAutosaveName を呼ぶ(ReaderWindowController.untileSavedWindowFrame)。フレーム欄は保存値のままなので、フィルで終了すれば次回も同じ見た目で起動し、外して終了すればその位置で起動する |
