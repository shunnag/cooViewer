...
| 圧縮ノイズ低減 | 旧実装には無い**新規**(既定オフ)。古い JPEG のブロックノイズ向け。設定「表示」の 2 軸: 強度(なし/弱/中/強。defaults `NoiseReductionLevel` 0-3)と適用範囲(メイン表示のみ/メイン表示とルーペ/原寸表示も含めて全て。`NoiseReductionScope`)。**JPEG のページのみ対象**(PNG 等の線画を甘くしないため拡張子判定)。弱(0.035/0.5)・中(0.06/0.35)は CINoiseReduction(Metal)。**強は CoreML**: waifu2x のノイズ除去モデル anime_noise2(MIT、約 1.2MB)を **初回選択時の同意後・必要時にのみ**配布元(imxieyi/waifu2x-mac)から DL し、SHA-256 ピン照合→コンパイル→Application Support/Models にキャッシュ(MLNoiseReducer actor。入力 3×142×142/出力 conv7、128px タイル+7px 文脈、1 タイル約 2ms)。未導入・失敗・XCTest 時は中相当の CI へフォールバック。設定の「強(ML モデル)」は初回に「約 1.2MB の DL+処理が重い」ことの確認ダイアログを出し、OK の場合のみ設定される(`NoiseReductionMLAccepted`)。導入状態は設定にキャプション表示(MLNoiseReducerStatus)。メイン表示は ImageResampler のリサンプル前段で適用しレベル込みのキーでキャッシュ(事前リサンプルも同一条件)、ルーペは超解像前、原寸はフル解像度デコード後に適用 |
