...
- JPEG の圧縮ノイズ低減(弱 / 中 / 強 / 最高。既定オフ。「強」は waifu2x のノイズ除去
  モデル(MIT License)、「最高」は Real-ESRGAN の ×4 超解像モデル(BSD-3-Clause)を
  初回同意のうえ必要時にダウンロードして CoreML で適用)
...
独自デコーダの形式(MAG / MAKI / Pi / PIC / PBM P4)は設定の「デコーダ」ペインで個別に無効化できます。
...
- 設定ウインドウは即時反映(旧: Cancel で全ロールバック)。システム設定風のサイドバー構成で、設定項目の検索もできます
